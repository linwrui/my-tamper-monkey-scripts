...
(async function () {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    promise = new Promise((resolve) =>
      setTimeout(() => {
        const colorFillSection = _.toArray(
          document.querySelectorAll("section")
        ).find((o) => o.firstChild.textContent === "填充");
        const colorItem =
          (colorFillSection &&
            colorFillSection.querySelector(
              '.item.items-group[data-label*="颜色"]'
            )) ||
          (colorFillSection &&
            colorFillSection.querySelector(
              '.item.items-group[data-label*="渐变"]'
            ));
        if (colorItem) {
          const isGradiant = colorItem
            .getAttribute("data-label")
            .includes("渐变");
          if (isGradiant) {
            let colors = [];
            const colorInputs = _.toArray(
              colorItem.querySelectorAll("input[data-color]")
            ).reverse();
